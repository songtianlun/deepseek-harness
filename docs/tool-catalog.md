...
Report the runtime environment the agent runs in: current working directory, operating system, platform, architecture, hostname, username, home directory, shell, Node.js version, and a small allowlist of environment variables. Call this before choosing paths or commands that depend on where the agent is running or on the host OS.
...
Source: [`packages/context/tool-environment/src/index.ts`](../packages/context/tool-environment/src/index.ts)
