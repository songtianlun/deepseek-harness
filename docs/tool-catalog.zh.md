...
| `@deepseek-ai/dsh-tool-environment` | `environment_info` | `ctx.tools`、`ctx.systemPrompt` | `tool/call`、`tool/result` | - | environment_info 报告宿主运行时——cwd、平台、操作系统名称/版本、架构、主机名、用户名、主目录、shell 与 Node 版本——以及一小份环境变量允许清单。允许清单是部署选择（Config.environmentVariables），具有安全默认值，因此除非部署方指名，否则不会报告任何机密；不存在的变量名为 null，以便模型区分「未设置」与「空字符串」。 |
...
<a id="deepseek-aidsh-tool-environment"></a>

## `@deepseek-ai/dsh-tool-environment`

### `environment_info`

报告 agent（智能体）运行所在的运行时环境：当前工作目录、操作系统、平台、架构、主机名、用户名、主目录、shell、Node.js 版本，以及一小份环境变量允许清单。在做出依赖运行位置或宿主操作系统的路径、命令决策前调用它。
...
来源：[`packages/context/tool-environment/src/index.ts`](../packages/context/tool-environment/src/index.ts)

environment_info 报告宿主运行时——cwd、平台、操作系统名称/版本、架构、主机名、用户名、主目录、shell 与 Node 版本——以及一小份环境变量允许清单。允许清单是部署选择（Config.environmentVariables），具有安全默认值，因此除非部署方指名，否则不会报告任何机密；不存在的变量名为 null，以便模型区分「未设置」与「空字符串」。
