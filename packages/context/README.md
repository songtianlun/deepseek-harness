...
Product plugins that add model-visible request context. Most add context without defining a tool; `tool-environment` reports the runtime environment to the model through one tool. `agent-instructions` is included by the default `dsh-agent-spine-demo` bundle and can be disabled through bundle config; the others are opt-in.
...
| [`tool-environment/`](tool-environment/README.md) | Model-facing `environment_info` reporting the host runtime | — |
